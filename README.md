...
|팀원 신정훈||
|팀원 백의헌|노션 세팅 및 회의록 작성<br>ERD 다이어그램 초안 작성<br>mapper, Service, Dao 초안 작성<br> API 아임포트 안전결제(결제, 결제취소, 결제수락, 결제확정)<br>거래상태 변경에 따른 상품상태 변경<br>안전거래 알림 생성, 삭제, 조회<br>상품 신고<br>주소(추가, 변경, 삭제)<br>리뷰 및 유저평점 업데이트 기능(프로시저 사용)<br>판매내역 조회<br>구매내역 조회<br>특정 유저 상품 조회<br>모아보기 페이지<br>찜 페이지<br>페이징 처리<br>|
#
## 📌 주요기능
