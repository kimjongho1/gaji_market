...
<body>
				...
				<c:forEach var="infacePurchaseInfo" items="${infacePurchaseList}">
					<div class="col mb-5">
						<div class="card h-100">
							...
							<c:choose>
								<c:when test="${not empty infacePurchaseInfo.url }">
									src="${infacePurchaseInfo.url}"
								</c:when>
								<c:otherwise>
  ...
  <path d="M11.5 4v-.5a3.5 3.5 0 1 0-7 0V4H1v10a2 2 0 0 0 2 2h10a2 2 0 0 0 2-2V4h-3.5ZM8 1a2.5 2.5 0 0 1 2.5 2.5V4h-5v-.5A2.5 2.5 0 0 1 8 1Zm0 6.993c1.664-1.711 5.825 1.283 0 5.132-5.825-3.85-1.664-6.843 0-5.132Z"/>
									...
									가격:<fmt:formatNumber value="${infacePurchaseInfo.price}" pattern="#,###"/>
								</div>
							</div>
							<!-- Product actions-->
		...
		<c:choose>
			<c:when test="${not empty searchWord}">
				...
				<c:if test="${endPageNum<totalPageNum}">
					<a
						href="<%=request.getContextPath()%>/mypage/orderstatus/inface?currentPage=${endPageNum+1}&searchWord=${searchWord}"><span>다음</span></a>
				</c:if>
			</c:when>
			<c:otherwise>
				<c:if test="${startPageNum!=1}">
				...
				<c:if test="${endPageNum<totalPageNum}">
					<a
						href="<%=request.getContextPath()%>/mypage/orderstatus/inface?currentPage=${endPageNum+1}"><span>다음</span></a>
				</c:if>
			</c:otherwise>
		</c:choose>
	</div>
