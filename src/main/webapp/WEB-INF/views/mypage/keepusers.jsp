		...
		<c:choose>
			<c:when test="${not empty searchWord}">
				...
				<c:if test="${endPageNum<totalPageNum}">
					<a
						href="<%=request.getContextPath()%>/mypage/keepusers?currentPage=${endPageNum+1}&searchWord=${searchWord}"><span>다음</span></a>
				</c:if>
			</c:when>
			<c:otherwise>
				<c:if test="${startPageNum!=1}">
					<a
						href="<%=request.getContextPath()%>/mypage/keepusers?currentPage=${startPageNum-1}"><span>이전</span></a>
				</c:if>
				<c:forEach begin="${startPageNum}" end="${endPageNum}" var="i">
					<a
				...
				<c:if test="${endPageNum<totalPageNum}">
					<a
						href="<%=request.getContextPath()%>/mypage/keepusers?currentPage=${endPageNum+1}"><span>다음</span></a>
				</c:if>
			</c:otherwise>
		</c:choose>
	</div>
